 ...
 * }
*/
function makePersonObject(givenId, givenName, givenEmail) {
  let person = {
    id: givenId,
    name: givenName,
    email: givenEmail
...
function getCarInfoByIndex(inventory, ind) {
  // const choice = inventory.find(( index));
  const carMake = inventory.find(index =>  index === inventory[ind])
  return `This is a ${carMake.car_make} ${carMake.car_model}`
  
  // return `This is a ${carMake} ${choice.car_model}`;
}
...
function getLastCarInfo(inventory) {
  const lengthLastCar = inventory.length -1;
  // const lastCar = inventory.find((item, index) => {
    // return inventory[lengthLastCar];
    // return index === lengthLastCar;
  // })
    const lastCar = inventory.find((item, index) => index === lengthLastCar)
  // const lastCar = inventory[lengthLastCar];
...
function getCarInfoById(inventory, ID) {
  const carID = inventory.find((element) =>  element.id === ID);
...
function sortCarInventory(inventory) {
  const sortInv = inventory.sort((a,b) => a.car_model.localeCompare(b.car_model));
  //review and check the action of localeCompare in MDN!!!! Now, do it now!!!!
  return sortInv;
}
...
function getModelYears(inventory) {
  ...
  for(let i=0;i<inventory.length;i++){
     modelYearsArray[i] = inventory[i].car_year;
  };
...
function getOlderCars(inventory, maxYear) {
  ...
  for(let i=0;i<inventory.length;i++){
    if(inventory[i].car_year <= maxYear){
      oldCars[oldCarIndex] = inventory[i];
      oldCarIndex++;
    };
 };
 return oldCars;
...
function getGermanCars(inventory) {
  const germanCars = [];
  let germanCarsIndex = 0;
  for(let i=0;i<inventory.length;i++){
    //Redo do this with a switch!!!!
    if(inventory[i].car_make ===  "Mercedes-Benz"|| inventory[i].car_make ==="Audi" ||inventory[i].car_make ==="Volkswagen"||inventory[i].car_make === "BMW"){
      germanCars[germanCarsIndex] = inventory[i];
      germanCarsIndex++;
    };
  };
  return germanCars;
...
function carMaker(odometerNum) {
  const car = {
    odometer: odometer = odometerNum, 
    drive: function(distanceDriven) {
